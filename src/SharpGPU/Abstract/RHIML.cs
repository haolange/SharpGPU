...
namespace SharpGPU
{
    ...
    /// sequence 鈥?a DAG encoded as ordered ops plus predecessor references, not a graph DSL.
    /// </summary>
    ...
    /// with (RFC-0003 搂4.1) to an arbitrary N-stage op sequence, and is the contract surface that
    /// lets a graph builder above the RHI lower a subgraph without taking a hard compile-time
    /// dependency on any backend's private operator-description types. This is not a graph DSL 鈥?it
    /// is a constructible execution primitive, at the same layer as <see cref="RHIMLEncoder"/> /
    /// <see cref="RHIBuffer"/> (RFC-0003 搂3.3 / ADR-0028).
    /// </summary>
