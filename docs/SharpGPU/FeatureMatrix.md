...
|---|---|---|
| Windows x64 | `docs/Artifacts/SharpGPU/feature-report-win-x64.json` | G4-T13 PASSED: regenerated OMM capability reflection; schema revision 5; pipeline ABI 8; serialization honest Unavailable. Matching-host CURRENT in `VERIFICATION.md`: Portable 183/183, WindowsQualified 19/19. Independent `OpacityMicromapVulkanQualifiedTests` 1/1. Full `SharpGpuVulkanQualified` still `BLOCKED_DEVICE` (ROAA). Capability reflection is not a substitute for those qualified gates |
