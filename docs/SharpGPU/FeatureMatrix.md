...
|---|---|---|
| Windows x64 | `docs/Artifacts/SharpGPU/feature-report-win-x64.json` | G4-T15 PENDING: regenerated Motion capability reflection; schema revision 6; pipeline ABI 9; SER capability absent. Matching-host counts in `VERIFICATION.md`: Portable 192/192, WindowsQualified 20/20. Independent `MotionVulkanQualifiedTests` 2/2. Full `SharpGpuVulkanQualified` still `BLOCKED_DEVICE` (ROAA). Capability reflection is not a substitute for those qualified gates |
