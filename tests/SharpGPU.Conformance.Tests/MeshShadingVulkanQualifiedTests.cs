...
{
    public sealed class MeshShadingVulkanQualifiedTests
    {
...
""";
        ...
        public void Vulkan_MeshDispatch_ShouldWriteDeterministicPixelOrFailClosed()
        {
            Assert.True(
            ...
            RHIDevice? meshDevice = null;
            RHIDevice? fallbackDevice = null;
            for (int i = 0; i < instance.DeviceCount; ++i)
            {
                ...
                if (candidate.Capabilities.Mesh.MeshShader.Tier != ERHICapabilityTier.Unavailable)
                {
                    meshDevice = candidate;
                    break;
                }
            }

            Assert.NotNull(fallbackDevice);
            if (meshDevice == null)
            {
                AssertUnavailableFailClosed(fallbackDevice);
                return;
            }

            DispatchMeshAndReadback(meshDevice);
        }

        private static void AssertUnavailableFailClosed(RHIDevice device)
        {
            ...
            RHICommandQueue queue =
                ...
                ?? throw new InvalidOperationException("The Vulkan graphics queue is unavailable.");
            using RHITexture target = CreateRenderTarget(device);
            using RHICommandBuffer commandBuffer = queue.CreateCommandBuffer();
            commandBuffer.Begin("G3.Vulkan.Mesh.Unavailable");
            ...
            {
                Name = "G3.Vulkan.Mesh.Unavailable",
                ArrayLength = 1,
            ...
            Assert.Throws<NotSupportedException>(() => raster.DispatchMesh(1, 1, 1));
            commandBuffer.EndRasterPass();
            commandBuffer.End();
        }

        private static void DispatchMeshAndReadback(RHIDevice device)
        {
            RHICommandQueue queue =
                device.GetCommandQueue(ERHIPipelineType.Graphics, 0)
                ?? throw new InvalidOperationException("The Vulkan graphics queue is unavailable.");
            ...
            commandBuffer.Begin("G3.Vulkan.MeshDispatch");
            PrepareRenderTarget(commandBuffer, target);
            RHIRasterEncoder raster = commandBuffer.BeginRasterPass(new RHIRasterPassDescriptor
            {
                Name = "G3.Vulkan.MeshDispatch",
                ArrayLength = 1,
                SampleCount = ERHISampleCount.None,
                ColorAttachments = new[]
                {
            ...
            string source)
        {
            ...
            try
            {
                result = HLSLCrossCompiler.Compile(
                    new ShaderCompileRequest
                    {
                        Source = source,
                        SourceName = "MeshShadingVulkanQualifiedTests.hlsl",
            ...
            {
                Assert.Fail(
                    $"SPIR-V compile failed for {stage}/{entryPoint}: {exception.Message}\n{exception.Diagnostics}");
                throw;
            }
