...
**更新** 绑定表实现已增强，采用统一的有限无界描述符后端，显著改进了描述符分配和管理效率。

- 作用：将 TLAS、着色器入口、采样器等绑定到管线状态
- 抽象：布局元素（槽位、数量、类型、阶段、是否必需）、元素（资源句柄）
- 后端实现：
  - **DX12**：使用分组化的描述符堆管理，支持延迟发布和脏区域跟踪
    - 通过 `Dx12BindingTableGroupStorage` 管理描述符组
    - 支持批量复制和优化的描述符更新
    - 内置空描述符缓存机制
  - **Vulkan**：基于描述符池分配器，支持动态描述符集租赁
    - 使用 `VulkanDescriptorSetLease` 管理描述符集生命周期
    - 支持按需更新和批量描述符写入
  - **Metal**：采用参数缓冲区策略，支持直接绑定和引用缓冲
    - 通过 `MetalArgumentBindingSnapshot` 捕获绑定状态
    - 支持直接绑定和间接引用两种模式
...
BT->>DX : 写入描述符范围(带脏标记)
BT->>VK : vkUpdateDescriptorSets(批量更新)
BT->>MT : 写入 Argument Buffer(快照捕获)
...
- 绑定表优化
  - **DX12**：利用分组化描述符堆和脏区域跟踪，减少不必要的描述符复制
  - **Vulkan**：使用描述符池分配器，支持批量更新和高效内存管理
  - **Metal**：采用参数缓冲区策略，减少 CPU-GPU 同步开销
  - 最小化必需绑定数量，避免多余描述符
  - 合理使用数组绑定与共享描述符范围
- 运动图形
  - 优先使用矩阵关键帧（Metal 限制）
...
## 结论
SharpGPU 通过抽象层统一了光线追踪的核心概念（BLAS/TLAS、几何体、OMM、绑定表），并在 DX12、Vulkan、Metal 三个后端实现了高效且一致的调用路径。**最新的绑定表实现采用了统一的有限无界描述符后端，显著提升了描述符分配和管理效率**。开发者可基于该抽象快速构建复杂场景的光线追踪渲染管线，同时利用后端特定的优化能力（如增量更新、运动图形、OMM）。在实际工程中，应重点关注几何与 OMM 的配置正确性、运动模式的兼容性、绑定表的必需项完整性，以及后端能力差异带来的限制与替代方案。
