...
## 更新摘要
**所做更改**
- 增强了后端选择机制的详细描述，包括平台检测、能力验证和运行时分发逻辑
- 完善了资源管理策略的说明，涵盖缓冲、纹理、堆等资源的创建和管理
- 添加了管线创建过程的详细分析，包括各种编码器（传输、计算、光线追踪、光栅化等）
- 扩展了能力域规范的内容，涵盖渲染、绑定、同步、内存、存储等多个能力领域
- 更新了架构图和数据流向图以反映最新的实现细节
...
- 实例创建：根据 descriptor.Backend 分发到对应后端实现类，支持编译时条件编译。
...
### 能力域规范与探测机制
SharpGPU 通过 RHIDeviceCapabilities 提供全面的能力域规范，涵盖以下主要领域：

- **渲染能力（RHIRasterCapabilities）**：投影策略、矩阵主序、深度值范围、多视图策略、像素着色器存储写入、帧缓冲区读写、各向异性采样、深度附件读取、帧缓冲区本地读取、间接绘制、可变速率着色、隐藏表面剔除、重心坐标、可编程采样位置、原生渲染通道、采样器反馈等。

- **绑定能力（RHIBindingCapabilities）**：根常量、间接根常量、原子操作、描述符索引、部分绑定描述符、绑定后更新描述符、空描述符等。

- **同步能力（RHISynchronizationCapabilities）**：时间戳查询、遮挡查询、管线统计查询、增强屏障、校准时间戳等。

- **内存能力（RHIMemoryCapabilities）**：统一内存、放置资源、GPU虚拟地址、稀疏缓冲、稀疏纹理、驻留性、预算查询、稀疏别名等。

- **存储能力（RHIStorageCapabilities）**：存储队列、变量速率着色、网格着色、工作图等。

- **管线缓存能力（RHIPipelineCacheCapabilities）**：原生管线缓存支持。

- **呈现能力（HIPresentationCapabilities）**：交换链、HDR支持等。

- **光线追踪能力（RHIRayTracingCapabilities）**：管线、内联、不透明度微映射、运动光线追踪等。

- **网格能力（RHIMeshCapabilities）**：网格着色器支持。

- **机器学习能力（RHIMachineLearningCapabilities）**：机器学习执行支持。

- **工作图能力（RHIWorkGraphCapabilities）**：执行、广播节点、线程节点、递归、网格节点、GPU输入、后备内存、入口记录等。

- **间接命令缓冲能力（RHIIndirectCommandBufferCapabilities）**：执行与令牌支持。

- **计算能力（RHIComputeCapabilities）**：波操作策略、波操作、可变子组大小、协作矩阵等。

- **函数库能力（RHIFunctionLibraryCapabilities）**：函数库支持。
...
- 能力不足：通过 RHIDeviceCapabilities 查询具体能力，未实现的功能会抛出 NotSupportedException。
...
- 能力域规范：通过 RHIDeviceCapabilities 查询具体设备能力，指导应用功能选择。